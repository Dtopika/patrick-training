...
function renderProgress(){
  ...
  $('#progressList').innerHTML=COMMANDS.map(c=>{const stats=commandTrialStats(c.cmd),recent=stats.avg===null?'Sin datos':`${Math.round(stats.avg*100)}% reciente`,evidence=commandContextEvidence(c.cmd),contextNote=STATE_SCORE[stateOf(c.cmd)]>=2?` · ${evidence.contextCount} ${evidence.contextCount===1?'contexto':'contextos'} validados`:'';return`<article class="progressRow progressRowV56"><div class="progressMain"><strong>${escapeHtml(displayCommand(c))} · <span class="pronunciation">${escapeHtml(displayPron(c))}</span></strong><small>Nivel ${c.level} · ${escapeHtml(c.meaning)}</small><div class="progressEvidence">${trialTrendHtml(stats.arr)}<span>${recent} · ${relativePracticeLabel(stats.lastMs)}${contextNote}</span></div></div><div class="progressRowActions"><button class="insightRowBtn" type="button" data-command-insight="${escapeHtml(c.cmd)}">Ficha</button><select data-state="${escapeHtml(c.cmd)}" aria-label="Estado de ${escapeHtml(displayCommand(c))}">${STATES.map(s=>`<option ${stateOf(c.cmd)===s?'selected':''}>${s}</option>`).join('')}</select></div></article>`}).join('');
