...
function applyStaticAppLanguage(){
  ...
  const dayLabel=$('.daySelect');if(dayLabel?.firstChild)dayLabel.firstChild.nodeValue=t('availability');const heroImg=$('.patrickHero img');if(heroImg)heroImg.alt=copyText('Ilustración de un cachorro pastor alemán para Patrick Training');if($('#dailyMissionConfidence')&&!currentDailyMissionPlan)$('#dailyMissionConfidence').textContent=copyText('Adaptativo v3');
