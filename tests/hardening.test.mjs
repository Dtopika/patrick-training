...
test('v6 bootstraps its dependencies when an older HTML shell loads newer JavaScript',async()=>{
  ...
  vm.runInContext(read('app-core.js'),env.ctx,{filename:'app-core.js'});
  await env.ctx.PATRICK_READY;
  assert.equal(vm.runInContext("CONFIG.APP_VERSION",env.ctx),'7.11.1');
  ...
  assert.deepEqual(env.scripts.map(s=>s.src),['config.js?v7111-r1','training-engine.js?v7111-r1','backup-schema.js?v7111-r1']);
});
...
test('configuration centralizes public schema and cache versions',()=>{
  ...
  assert.equal(env.ctx.PATRICK_CONFIG.APP_VERSION,'7.11.1');
  ...
  assert.equal(JSON.parse(read('package.json')).version,'7.11.1');
  assert.equal(JSON.parse(read('package-lock.json')).version,'7.11.1');
});
...
test('backup schema accepts 5.x backups, v6 schema, and rejects malformed nested data',()=>{
  ...
  const current=schema.normalize({schemaVersion:13,appVersion:'7.11.1',currentLevel:0,dayType:'Solo noche',history:[]},opts);
...
test('v7 adaptive engine uses precise timing, skill profiles and adaptive repetitions',()=>{
  const engine=read('training-engine.js'),session=read('app-session.js'),insights=read('app-insights.js'),index=read('index.html');
  assert.match(engine,/function skillFamily/);
  assert.match(engine,/function difficultyTarget/);
  assert.match(engine,/function commandTimingEvidence/);
  assert.match(engine,/function evidenceConfidence/);
  assert.match(engine,/function recommendedAttempts/);
  assert.match(engine,/timingMode!=='cue-to-rating'/);
  assert.match(session,/timingMode:'cue-to-rating'/);
  assert.match(session,/function beginExecution/);
  assert.match(session,/executionReadyForRating/);
  assert.match(session,/targets:\{\}/);
  assert.match(session,/ENGINE\.recommendedAttempts/);
  assert.match(insights,/smartPlanBadge">v3/);
  assert.match(insights,/commandInsightConfidence/);
  assert.match(index,/id="commandInsightConfidence"/);
...
test('v7.3 first-run wizard configures theme profile tutorial and level zero before app use',()=>{
  ...
  assert.match(index,/data-setup-theme="system"/);
  assert.match(index,/id="setupDogName"/);
  assert.match(index,/id="setupDogAge"/);
  assert.match(profile,/SETUP_WIZARD_VERSION=1/);
  assert.match(profile,/function openSetupWizard/);
  assert.match(profile,/function completeSetupWizard/);
  assert.match(profile,/currentLevel===0&&!completeProfile/);
  assert.match(profile,/patrickSetupWizardVersion/);
  assert.match(core,/patrickSetupWizardVersion:0/);
  assert.match(backup,/setupWizardVersion/);
});

test('v7.11.1 destructive actions use app-native confirmations and reset stays double-gated',()=>{
...
test('v7.5 separates app language from command language without changing canonical ids',()=>{
  ...
  assert.match(sw,/i18n\.js\?v7111-r1/);
});
...
test('v7.11.1 keeps management reachable, locks background and makes update checks observable',()=>{
