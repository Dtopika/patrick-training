...
test('v6 bootstraps its dependencies when an older HTML shell loads newer JavaScript',async()=>{
  ...
  vm.runInContext(read('app-core.js'),env.ctx,{filename:'app-core.js'});
  await env.ctx.PATRICK_READY;
  assert.equal(vm.runInContext("CONFIG.APP_VERSION",env.ctx),'7.3.0');
  ...
  assert.deepEqual(env.scripts.map(s=>s.src),['config.js?v730-r1','training-engine.js?v730-r1','backup-schema.js?v730-r1']);
});
...
test('v7.3 configuration centralizes public and schema versions',()=>{
  const env=baseContext();loadArchitecture(env.ctx);
  assert.equal(env.ctx.PATRICK_CONFIG.APP_VERSION,'7.3.0');
  assert.equal(env.ctx.PATRICK_CONFIG.BACKUP_SCHEMA_VERSION,12);
  assert.equal(env.ctx.PATRICK_CONFIG.SESSION_SCHEMA_VERSION,9);
  assert.equal(env.ctx.PATRICK_CONFIG.CACHE_NAME,'patrick-training-v7.3.0-r1');
...
test('backup schema accepts 5.x backups, v6 schema, and rejects malformed nested data',()=>{
  ...
  const current=schema.normalize({schemaVersion:12,appVersion:'7.3.0',currentLevel:0,dayType:'Solo noche',history:[]},opts);
...
test('v7.3 upgrade contract cache-busts every critical browser asset',()=>{
